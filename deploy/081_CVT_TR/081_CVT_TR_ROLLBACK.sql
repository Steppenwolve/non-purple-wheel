/* ============================================================================
   081_CVT_TR_ROLLBACK.sql
   Fin     : Revertir los cambios aplicados por 081_CVT_TR_CORRECCION.sql.
   IMPORTANTE:
...
BEGIN
    ...
    --EXEC [dbo].[081_CVT TR] @FECHA = '20240420'

    -- Para reportes diarios WHERE FECHA_REPORTE = @Fecha
END;
GO
PRINT '>> ION.dbo.[081_CVT TR] revertido al estado original.';
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (
            SELECT ID FROM [SILVER].[RR].[081_CVT TR]
            WHERE [FECHATRANSFERENCIA] = @FechaSistema
        )
        BEGIN
...
END;
GO
PRINT '>> SILVER.dbo.[081_CVT TR] revertido al estado original.';
GO
...
BEGIN
    ...
    PRINT '>> Columna FECHA_INFO eliminada de SILVER.[RR].[081_CVT TR].';
END
ELSE
    PRINT '>> FECHA_INFO no existe. Sin cambios.';
GO

PRINT '>> Rollback 081_CVT_TR completado.';
GO
