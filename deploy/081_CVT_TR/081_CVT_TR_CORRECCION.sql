/* ============================================================================
   081_CVT_TR_CORRECCION.sql
   Reporte : CVT_TR  (Custodia / Transferencias - Diaria)
   Fin     : Corregir hallazgos detectados vs. Layout_CVT_TR_V3.xlsx:
               1. SILVER.RR.[081_CVT TR] - ADD FECHA_INFO date
               2. SILVER.dbo.[081_CVT TR] - eliminar ventana mensual
                  (codigo muerto), usar @FechaDia, incluir FECHA_INFO
               3. ION.dbo.[081_CVT TR]   - remover ID y FECHA_EXTRACCION,
...
IF NOT EXISTS (
    ...
    WHERE s.name = 'RR' AND o.name = '081_CVT TR' AND o.type = 'U'
)
BEGIN
    RAISERROR('Preflight fallido: SILVER.[RR].[081_CVT TR] no existe. Abortando.', 16, 1);
    RETURN;
END
PRINT '>> Preflight OK: SILVER.[RR].[081_CVT TR] existe.';
GO
...
IF NOT EXISTS (
    ...
    WHERE TABLE_SCHEMA = 'RR' AND TABLE_NAME = '081_CVT TR' AND COLUMN_NAME = 'FECHA_INFO'
)
BEGIN
    ALTER TABLE [RR].[081_CVT TR]
        ADD [FECHA_INFO] date NULL;
    PRINT '>> Columna FECHA_INFO date agregada a SILVER.[RR].[081_CVT TR].';
END
ELSE
...
GO
CREATE OR ALTER PROCEDURE [dbo].[081_CVT TR]
...
BEGIN
    ...
    DECLARE @NombreJob       NVARCHAR(128) = '[081_CVT TR]';
    DECLARE @FechaDia        DATE          = CAST(@FechaSistema AS DATE);

    BEGIN TRY
        IF EXISTS (
            SELECT 1 FROM [SILVER].[RR].[081_CVT TR]
            WHERE [FECHATRANSFERENCIA] = @FechaDia
        )
        BEGIN
            DELETE FROM [SILVER].[RR].[081_CVT TR]
            WHERE [FECHATRANSFERENCIA] = @FechaDia;
        ...
        INSERT INTO [RR].[081_CVT TR] (
            [CLIENTEINSTITUCION],
            [FECHANOTIFICACIONTRANSFERENCIA],
            [TIPOTRANSFERENCIACUSTODIA],
            [FECHATRANSFERENCIA],
            [TITULOTRANSFERENCIA],
            [NUMEROTITULOS],
            [CUSTODIOCONTRAPARTE],
            [NUMEROIDENTIFICACIONTRANSFERENCIA],
            [TIPOMODIFICACION],
            [FECHA_INFO]
        )
        SELECT
...
END;
GO
PRINT '>> SILVER.dbo.[081_CVT TR] actualizado.';
GO
...
BEGIN
    ...
    SELECT
        [CLIENTEINSTITUCION],
        FORMAT([FECHANOTIFICACIONTRANSFERENCIA], 'yyyy/MM/dd') AS [FECHANOTIFICACIONTRANSFERENCIA],
        [TIPOTRANSFERENCIACUSTODIA],
        FORMAT([FECHATRANSFERENCIA], 'yyyy/MM/dd')             AS [FECHATRANSFERENCIA],
        [TITULOTRANSFERENCIA],
        [NUMEROTITULOS],
        [CUSTODIOCONTRAPARTE],
        [NUMEROIDENTIFICACIONTRANSFERENCIA],
    ...
    FROM [SILVER].[RR].[081_CVT TR]
    WHERE [FECHATRANSFERENCIA] = @FECHA;
END;
GO
PRINT '>> ION.dbo.[081_CVT TR] actualizado.';
PRINT '>> Correccion 081_CVT_TR completada.';
GO
